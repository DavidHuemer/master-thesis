...
class TestConsistencyTestCaseBuilder(TestCase):
    def setUp(self):
        ...
        self.consistency_test_case_builder = ConsistencyTestCaseBuilder(self.expected_result_finder)

    def test_build_test_cases_with_empty_arrays(self):
        expected_results = []
        java_code_list = []

        consistency_test_cases = self.consistency_test_case_builder.build_test_cases(expected_results,
                                                                                       java_code_list)
        self.assertEqual(consistency_test_cases, [])
    ...
    def test_build_with_only_java_code(self):
        ...
        self.consistency_test_case_builder.build_test_case = Mock()
        expected_test_case = ConsistencyTestCase(java_code, java_code.methods[0], None)
        self.consistency_test_case_builder.build_test_case.return_value = expected_test_case

        consistency_test_cases = self.consistency_test_case_builder.build_test_cases(expected_results,
                                                                                       java_code_list)
        self.assertEqual(consistency_test_cases, [expected_test_case])
        self.consistency_test_case_builder.build_test_case.assert_called_once_with(java_code, java_code.methods[0],
                                                                                   expected_results)
    ...
    def test_build_test_case_with_no_expected_results(self):
        ...
        consistency_test_case = self.consistency_test_case_builder.build_test_case(java_code, method,
                                                                                     expected_results)
        self.assertEqual(consistency_test_case, ConsistencyTestCase(java_code, method, None))
    ...
    def test_build_test_case_with_expected_results(self):
        ...
        consistency_test_case = self.consistency_test_case_builder.build_test_case(java_code, method,
                                                                                     expected_results)
        self.assertEqual(consistency_test_case, ConsistencyTestCase(java_code, method, expected_results[0]))
        self.expected_result_finder.get_expected_result_for_code.assert_called_once_with(java_code, method,
                                                                                         expected_results)
